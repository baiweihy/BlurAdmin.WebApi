...
namespace LegacyStandalone.Web.Controllers.HumanResources
{
    ...
    public class EmployeeController : ApiControllerBase
    {
        ...
        {
            var models = await _employeeRepository.AllIncluding(x => x.JobPost).ToListAsync();
        ...
        {
            var model = await _employeeRepository.GetSingleAsync(x => x.Id == id, x => x.JobPost);
            if (model != null)
            {
        ...
        {
            var exp = _employeeRepository.AllIncluding(x => x.Department, x => x.JobPost).AsQueryable();
